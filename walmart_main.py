...
URL = "https://www.walmart.com/ip/Noxzema-Classic-Clean-Original-Deep-Cleansing-Cream-12-oz/10294073"
OUTPUT_JSON = "output/walmart_city.json"
OUTPUT_CSV = "output/walmart_city.csv"
STATE_DIR = "state/walmart_profile"
...
class WalmartScraper:
    ...
    # STEP 6: Save debug screenshot and HTML
    ...
    # STEP 7: Detect block/error pages
    ...
    # STEP 8: Validate page really looks like a product page
    ...
    # STEP 9: Wait for manual resolution if needed
    ...
    def extract_ingredients_from_text(self, text: str) -> Optional[str]:
        match = re.search(
            r"ingredients[:\s]+(.+?)(?:directions|warning|about this item|$)",
            text,
            re.I | re.S
        )
        if match:
    ...
    # STEP 13: Reject non-product images
    ...
    def scrape(self, url: str) -> None:
        ...
        with sync_playwright() as p:
            ...
            self.save_debug_files(page, "walmart_debug")

            if self.is_blocked(page):
                self.wait_for_manual_resolution(page, seconds=90)
                self.save_debug_files(page, "walmart_debug_after_wait")
            ...
            # STEP 16: Extract only after page is confirmed real
            product_name = (
            ...
            price = (
                json_ld.get("price")
                or self.safe_text(page, '[itemprop="price"]')
            ...
            description = (
                ...
                or self.safe_text(page, 'text=/deep cleans/i')
            )
